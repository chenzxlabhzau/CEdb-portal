...
export class SidePanelComponent implements OnInit  {
  ...
  @Output() $selectpeak = new EventEmitter<any>();
  ...
  peaks:string[] = ["Typical enhancer","Super enhancer"];
  // selectedOptions: string[] = ["B-Cell Lymphoma"];
  selectedOptions:string[] = ["BC"];
  ...
  public peakSelection(selections : any): void {
    let selected:string[] = []
    for (let entry of selections) {
      selected.push(entry.value)
    }
    this.$selectpeak.emit(selected)
  }
