...
export class sampleDataSource implements DataSource<sampleRecord> {
  ...
  loadsampleRecords(cancer_type: string[], source_type:string[],query:string, sortOrder: string, pageIndex: number, pageSize: number) {
    this.loadingSubject.next(true);

    this.dataApiService
      .findsampleRecords(cancer_type, source_type,query, sortOrder, pageIndex, pageSize)
      .pipe(
        tap((val) => {
